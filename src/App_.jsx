...
// ============================================================
// MODULE: alunoResumos — Resumos escritos pelos alunos
// ============================================================
const alunoResumosModule = {
  save(alunoId, planoId, topicId, tipo, resumo) {
    storage.set(db => {
      const resumos = db.alunoResumos || [];
      const idx = resumos.findIndex(r => r.alunoId === alunoId && r.planoId === planoId && r.topicId === topicId && r.tipo === tipo);
      if (idx >= 0) {
        const updated = [...resumos];
        updated[idx] = { ...updated[idx], resumo, atualizado: new Date().toISOString() };
        return { ...db, alunoResumos: updated };
      }
      return { ...db, alunoResumos: [...resumos, { alunoId, planoId, topicId, tipo, resumo, criado: new Date().toISOString(), atualizado: new Date().toISOString() }] };
    });
  },
  get(alunoId, planoId, topicId, tipo) {
    return (storage.get().alunoResumos || []).find(r => r.alunoId === alunoId && r.planoId === planoId && r.topicId === topicId && r.tipo === tipo)?.resumo || "";
  },
  getAll(alunoId, planoId, topicId) {
    return (storage.get().alunoResumos || []).filter(r => r.alunoId === alunoId && r.planoId === planoId && r.topicId === topicId);
  },
};
              ...
              <div style={{ display: "grid", gap: 12, marginBottom: 20 }}>
                ...
                <div style={{ padding: 12, borderRadius: 8, background: "var(--s2)" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                    {(() => {
                      const stats = planosModule.getStats(selectedAlunoId, alunoPlanoAtivo.id);
                      return (
                        <>
...
export default function App() {
  ...
  function renderPage() {
    ...
    if (user.role === "aluno") {
      if (page==="dashboard")      if (page==="plano")     return <AlunoPlano     user={user} refresh={refresh}/>;
  ...
  return (
    ...
    >
      {user ? renderPage() : <LoginPage onLogin={setUser} />}
    </Layout>
  );
}
