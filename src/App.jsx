...
const resumoModule = {
  ...
  // Resumos do aluno
  save(resumoId, alunoId, topicId, titulo, conteudo, materiaId) {
    storage.set(db => {
      const resumos = db.alunoResumos || [];
      const idx = resumos.findIndex(r => r.id === resumoId);
      if (idx >= 0) {
        const updated = [...resumos];
        updated[idx] = { ...updated[idx], titulo, conteudo, materiaId, updatedAt: new Date().toISOString() };
        return { ...db, alunoResumos: updated };
      }
      const novoResumo = {
        id: `r-${Date.now()}`,
        alunoId,
      ...
      return { ...db, alunoResumos: [...resumos, novoResumo] };
    });
  },
  delete(resumoId) {
    storage.set(db => ({
      ...db,
      alunoResumos: (db.alunoResumos || []).filter(r => r.id !== resumoId)
    }));
  },
  getByAluno(alunoId) {
    return (storage.get().alunoResumos || []).filter(r => r.alunoId === alunoId);
  },
  getByTopic(alunoId, topicId) {
    return (storage.get().alunoResumos || []).find(r => r.alunoId === alunoId && r.topicId === topicId) || null;
  },
  getByMateria(alunoId, materiaId) {
    return (storage.get().alunoResumos || []).filter(r => r.alunoId === alunoId && r.materiaId === materiaId);
  }
};
...
// ============================================================
// ALUNO: Resumos — Visualizar, editar e exportar resumos
// ============================================================
function AlunoResumos({ user, refresh }) {
  ...
  if (!plano || !edital) {
    return (
      <div>
        <div className="ph"><div><h1>📓 Resumos</h1><p>Seus resumos de estudo</p></div></div>
        <div className="card"><div className="empty"><h3>Nenhum plano ativo</h3><p>Gere um plano para criar resumos.</p></div></div>
      </div>
    );
  }
  ...
  edital.materias?.forEach(m => {
    resumosPorMateria[m.id] = {
      nome: m.name,
      color: m.color,
      resumos: resumos.filter(r => r.materiaId === m.id)
    };
  });

  const handleEditClick = (resumo) => {
    setEditingId(resumo.id);
    setEditForm({ titulo: resumo.titulo, conteudo: resumo.conteudo });
  };

  const handleSave = () => {
    if (editForm.titulo && editForm.conteudo) {
      const resumo = resumos.find(r => r.id === editingId);
      if (resumo) {
        resumoModule.save(editingId, user.id, resumo.topicId, editForm.titulo, editForm.conteudo, resumo.materiaId);
        setEditingId(null);
  ...
  const handleExportPDF = (resumoId) => {
    ...
    const html = `
      ...
      <body>
        <h1>${resumo.titulo}</h1>
        <div class="meta">
          <p>Data: ${new Date(resumo.updatedAt).toLocaleDateString('pt-BR')}</p>
          <p>Gerado em: ${new Date().toLocaleString('pt-BR')}</p>
        </div>
        <div class="content">${resumo.conteudo}</div>
      </body>
      </html>
    `;
  ...
  const handleExportDOCX = async (resumoId) => {
    const resumo = resumos.find(r => r.id === resumoId);
    if (!resumo) return;

    // Criar XML simples para DOCX
    const docContent = `<?xml version="1.0" encoding="UTF-8"?>
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:body>
    <w:p>
      <w:r>
        <w:t>${resumo.titulo}</w:t>
      </w:r>
    </w:p>
    <w:p><w:r><w:t></w:t></w:r></w:p>
    <w:p>
      <w:r>
        <w:t>${resumo.conteudo.replace(/\n/g, '</w:t></w:r></w:p><w:p><w:r><w:t>')}</w:t>
      </w:r>
    </w:p>
  </w:body>
</w:document>`;

    // Simples fallback: exportar como texto
    const txtContent = `${resumo.titulo}\n\n${resumo.conteudo}`;
    const blob = new Blob([txtContent], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${resumo.titulo.replace(/\s+/g, '_')}.txt`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const mateirasSemResumo = Object.entries(resumosPorMateria).filter(([_, data]) => data.resumos.length === 0);
      ...
      {showExportModal && (
        <div className="overlay" onClick={() => setShowExportModal(false)}>
          <div className="modal fi" style={{ maxWidth: 400, padding: "30px" }} onClick={e => e.stopPropagation()}>
            <div className="modal-hd" style={{ marginBottom: "20px" }}>
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Exportar Resumo</h2>
            </div>
            <p style={{ marginBottom: "20px", color: "var(--t3)", fontSize: 14 }}>Escolha o formato para download:</p>
            <div style={{ display: "flex", gap: 12, flexDirection: "column" }}>
              <button
                className="btn btn-green"
                onClick={() => {
                  handleExportPDF(exportResumoId);
              ...
              <button
                ...
                onClick={() => {
                  handleExportDOCX(exportResumoId);
                  setShowExportModal(false);
                }}
                style={{ width: "100%" }}
              >
      ...
      {resumos.length === 0 ? (
        ...
        <>
          <div className="g3 mb4">
            <div className="stat"><div className="stat-l">Total de Resumos</div><div className="stat-v">{resumos.length}</div></div>
            <div className="stat"><div className="stat-l">Matérias com Resumos</div><div className="stat-v">{Object.entries(resumosPorMateria).filter(([_, d]) => d.resumos.length > 0).length}</div></div>
            <div className="stat"><div className="stat-l">Última Atualização</div><div className="stat-v" style={{ fontSize: 12, marginTop: 6 }}>{new Date(Math.max(...resumos.map(r => new Date(r.updatedAt)))).toLocaleDateString('pt-BR')}</div></div>
          </div>

          <div className="card">
            <div className="card-title" style={{ marginBottom: 16 }}>Resumos por Matéria</div>

            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
              <button
                className={`preset-btn${!selectedMateria ? " active" : ""}`}
                onClick={() => setSelectedMateria(null)}
              >
                Todas ({resumos.length})
              </button>
              {Object.entries(resumosPorMateria).map(([materiaId, data]) =>
                data.resumos.length > 0 && (
                  <button
                    key={materiaId}
                    className={`preset-btn${selectedMateria === materiaId ? " active" : ""}`}
                    onClick={() => setSelectedMateria(materiaId)}
                  >
                    {data.nome} ({data.resumos.length})
                  </button>
                )
              )}
            </div>
            ...
            <div style={{ display: "grid", gap: 16 }}>
              {Object.entries(resumosPorMateria).map(([materiaId, data]) => {
                const resumosFiltrados = selectedMateria ? data.resumos : resumos.filter(r => r.materiaId === materiaId);
                if (!selectedMateria && resumosFiltrados.length === 0) return null;
                if (selectedMateria && materiaId !== selectedMateria) return null;

                return (
                  <div key={materiaId}>
                    {!selectedMateria && (
                      <h3 style={{ margin: "12px 0", fontSize: 14, fontWeight: 700, color: "var(--t2)", borderLeft: `3px solid ${data.color}`, paddingLeft: 12 }}>
                        {data.nome}
                      </h3>
                    )}
                    <div style={{ display: "grid", gap: 12 }}>
                      {resumosFiltrados.map(resumo => (
                        editingId === resumo.id ? (
                          <div key={resumo.id} style={{ padding: 16, background: "var(--s2)", borderRadius: 8, border: "1px solid var(--b2)" }}>
                            <input
                              type="text"
                            ...
                            />
                            <div style={{ display: "flex", gap: 8 }}>
                              <button className="btn btn-green btn-sm" onClick={handleSave}>✓ Salvar</button>
                              <button className="btn btn-ghost btn-sm" onClick={() => setEditingId(null)}>✕ Cancelar</button>
                            </div>
                          </div>
                        ) : (
                          <div key={resumo.id} style={{ padding: 16, background: "var(--s1)", borderRadius: 8, border: "1px solid var(--b1)" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                              <div>
                                <h4 style={{ margin: "0 0 4px 0", fontSize: 15, fontWeight: 700 }}>{resumo.titulo}</h4>
                                <div style={{ fontSize: 11, color: "var(--t3)" }}>
                                  Atualizado em {new Date(resumo.updatedAt).toLocaleDateString('pt-BR')}
                                </div>
                              </div>
                              <div style={{ display: "flex", gap: 6 }}>
                                <button
                                  className="btn btn-sm btn-blue"
                                  onClick={() => handleEditClick(resumo)}
                                  title="Editar resumo"
                                >
                                  ✏️
                                </button>
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (window.confirm("Excluir este resumo?")) {
                                      resumoModule.delete(resumo.id);
                                      refresh();
                                    }
                                  }}
                            ...
                            <div style={{ fontSize: 13, color: "var(--t2)", whiteSpace: "pre-wrap", wordBreak: "break-word", maxHeight: 150, overflow: "hidden" }}>
                              {resumo.conteudo}
                            </div>
                            {resumo.conteudo.length > 150 && (
                              <div style={{ fontSize: 11, color: "var(--t3)", marginTop: 8 }}>... [resumo truncado]</div>
                            )}
                          </div>
                        )
                      ))}
          ...
          {mateirasSemResumo.length > 0 && (
            <div className="card mt4">
              <div className="card-title" style={{ marginBottom: 12 }}>⚡ Matérias Sem Resumos</div>
              <p style={{ color: "var(--t3)", fontSize: 13, marginBottom: 12 }}>Crie resumos para as seguintes matérias:</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12 }}>
                {mateirasSemResumo.map(([materiaId, data]) => (
                  <div key={materiaId} style={{ padding: 12, background: "var(--s2)", borderRadius: 6, borderLeft: `3px solid ${data.color}` }}>
                    <div style={{ fontWeight: 600, fontSize: 13 }}>{data.nome}</div>
                  </div>
                ))}
              </div>
            </div>
